...
import { parseListPanes, validateTarget } from '#lib/tmux'
...
const tools = await import('../src/mcp/tools').then((m) => m.default)
...
const testBasicErrorBehavior = <T, P>(
    handler: (params: P) => Promise<T>,
    params: P,
) => [
    it('returns error when tmux not installed', async function () {
        setupMocks(false)
        const result = await handler(params)
        expect(mcpError(result)).toContain('not installed')
    }),
    it('returns error when no session', async function () {
...
describe('tmux_list', () => {
    ...
    it('filters by session', async () => {
        ...
        const result = await handler({ session: 'work' })
    ...
    testBasicErrorBehavior(handler, {})
})

// ─── tmux_read ───

describe('tmux_read', () => {
    const { handler } = tools.read
    ...
    it('captures visible pane content', async () => {
        ...
        const result = await tools.read.handler({ target: 'dev:0.0' })
    ...
    it('uses custom line count', async () => {
        ...
        await tools.read.handler({ target: 'dev:0.0', lines: 100 })
        const args = mockExec.mock.calls[0][0]
    ...
    it('returns error for invalid target', async () => {
        setupMocks()
        const result = await tools.read.handler({ target: ';rm -rf /' })
        expect(mcpError(result)).toContain('Invalid tmux target')
    })

    testBasicErrorBehavior(handler, { target: 'dev:0.0' })
})

// ─── tmux_send ───

describe('tmux_send', () => {
    const { handler } = tools.send

    it('sends literal text with enter', async () => {
        setupMocks()
        mockExec.mockResolvedValue({ stdout: '', exitCode: 0 })
        const result = await handler({ target: 'dev:0.0', keys: 'npm test' })
        mcpText(result)
    ...
    it('handles special key names literally via -l', async () => {
        setupMocks()
        mockExec.mockResolvedValue({ stdout: '', exitCode: 0 })
        await handler({ target: 'dev:0.0', keys: 'Enter', enter: false })
    ...
    it('returns error for invalid target', async () => {
        setupMocks()
        const result = await handler({ target: '$(whoami)', keys: 'test' })
        expect(mcpError(result)).toContain('Invalid tmux target')
    })

    testBasicErrorBehavior(handler, { target: 'dev:0.0', keys: 'test' })
})

// ─── tmux_split ───

describe('tmux_split', () => {
    const { handler } = tools.split

    it('splits vertically by default and returns new target', async () => {
        setupMocks()
        mockExec.mockResolvedValue({ stdout: 'dev:0.1', exitCode: 0 })
        const result = await handler({ target: 'dev:0.0' })
        const text = mcpText(result)
        expect(text).toContain('dev:0.1')
        const args = mockExec.mock.calls[0][0]
    ...
    it('splits horizontally when specified', async () => {
        setupMocks()
        mockExec.mockResolvedValue({ stdout: 'dev:0.1', exitCode: 0 })
        await handler({ target: 'dev:0.0', direction: 'horizontal' })
        const args = mockExec.mock.calls[0][0]
    ...
    it('passes percent and command', async () => {
        setupMocks()
        mockExec.mockResolvedValue({ stdout: 'dev:0.1', exitCode: 0 })
        await handler({ target: 'dev:0.0', percent: 30, command: 'htop' })
        const args = mockExec.mock.calls[0][0]
...
describe('tmux_close', () => {
    ...
    it('closes a pane', async () => {
        ...
        const result = await handler({ target: 'dev:0.1' })
        mcpText(result)
    })
    ...
    it('returns error for invalid target', async () => {
        setupMocks()
        const result = await handler({ target: ';rm -rf /' })
        expect(mcpError(result)).toContain('Invalid tmux target')
    })
