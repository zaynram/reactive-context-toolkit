import type { LangTool } from '#config/types'
import { fs, xml, entries } from '#util'

export default {
    name: 'bun',
    config: fs.resolve('bunfig.toml'),
    manifest: fs.resolve('package.json'),
} as LangTool

export function getBunScripts(tool: LangTool, cwd: string): string {
    const manifestPath = tool.manifest ?? fs.join(cwd, 'package.json')
    try {
        const pkg = JSON.parse(fs.readRaw(manifestPath))
        const scripts = pkg?.scripts as Record<string, string> | undefined
        if (!scripts || Object.keys(scripts).length === 0) return xml.inline('scripts')

        const inner = entries<string>(scripts)
            .map(([name, command]) => xml.inline('script', { name, command }))
            .join('')
        return xml.wrap('scripts', { inner })
    } catch {
        return xml.inline('scripts')
    }
}

export function getBunWorkspace(tool: LangTool, cwd: string): string {
    const manifestPath = tool.manifest ?? fs.join(cwd, 'package.json')
    try {
        const pkg = JSON.parse(fs.readRaw(manifestPath))
        const workspaces = pkg?.workspaces as string[] | undefined
        if (!workspaces || workspaces.length === 0) return xml.inline('workspaces')

        const inner = workspaces.map(w => xml.inline('workspace', { path: w })).join('')
        return xml.wrap('workspaces', { inner })
    } catch {
        return xml.inline('workspaces')
    }
}
