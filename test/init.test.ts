...
describe("mergeSettings", () => {
  ...
  test("adds hooks without overwriting existing settings", () => {
    setup({})
    const settingsDir = path.join(TMP_DIR, ".claude")
    mkdirSync(settingsDir, { recursive: true })
    const settingsPath = path.join(settingsDir, "settings.json")
    writeFileSync(settingsPath, JSON.stringify({ permissions: { allow: ["Read"] } }, null, 2))
  ...
  test("creates settings file if it does not exist", () => {
    setup({})
  ...
  test("adds PreToolUse and PostToolUse hooks when rules/injections have matchers", () => {
    setup({})
    const settingsPath = path.join(TMP_DIR, ".claude", "settings.json")

    mergeSettings(settingsPath, {
      rules: [
        { on: "PreToolUse", matcher: "Write|Edit", match: { target: "file_path", pattern: ".*" }, action: "block", message: "no" },
      ],
      injections: [
  ...
  test("is idempotent -- running twice produces same result", () => {
    ...
    const config = {
      rules: [
        { on: "PreToolUse" as const, matcher: "Write", match: { target: "file_path" as const, pattern: ".*" }, action: "block" as const, message: "no" },
      ],
    }
