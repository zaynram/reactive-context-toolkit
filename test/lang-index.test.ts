...
describe("evaluateLang", () => {
    test("returns bun scripts for configured bun tool", () => {
        ...
        const results = evaluateLang(lang, "SessionStart", fixtureDir, defaultGlobals)
    ...
    test("skips tools not matching event", () => {
        const lang: LangConfig = {
            typescript: {
                tools: [
                    { name: "bun", scripts: true, injectOn: "PreToolUse" },
                ],
            },
        }
        const results = evaluateLang(lang, "SessionStart", fixtureDir, defaultGlobals)
    ...
    test("uses lang-level injectOn as default", () => {
        ...
        const results = evaluateLang(lang, "PreToolUse", fixtureDir, defaultGlobals)
        expect(results.length).toBeGreaterThan(0)
    })
