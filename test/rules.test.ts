...
describe('evaluateRules', () => {
    test('returns null when no rules match', () => {
        const result = evaluateRules([blockRule], 'PreToolUse', 'Write', {
    ...
    test('skips rules with enabled: false', () => {
        ...
        const result = evaluateRules([disabled], 'PreToolUse', 'Write', {
            tool_input: { file_path: 'docs/specs/api.md' },
        })
        expect(result).toBeNull()
    })
    ...
    test('filters by event name', () => {
        const result = evaluateRules([blockRule], 'PostToolUse', 'Write', {
            tool_input: { file_path: 'docs/specs/api.md' },
        })
        expect(result).toBeNull()
    })
    ...
    test('filters by matcher (pipe-delimited tool names)', () => {
        const result = evaluateRules([blockRule], 'PreToolUse', 'Read', {
            tool_input: { file_path: 'docs/specs/api.md' },
        })
        expect(result).toBeNull()
    })
    ...
    test('matches when no matcher specified (any tool)', () => {
        ...
        const result = evaluateRules([noMatcher], 'PreToolUse', 'Write', {
